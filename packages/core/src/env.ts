...
const schema = z.object({
   ...
   *
   * ═══ ЗАЩО КЛЮЧ, А НЕ ИЗТРИТ КОД ═══
   *
   * Галерията се скрива, докато приложението се пуска — в нея има проби от
   * тестване и не бива да са първото, което вижда нов човек. Това е решение
   * за ДЕНЯ, не за винаги.
   *
   * Изтрит код значи писане наново след месец. Ключ значи една променлива
   * и едно пускане. Затова кодът остава цял, а видимостта е настройка.
   *
   * ⚠ Изключено НЕ значи „скрито от очите". Заявките към `/api/lookbook`
   *   също отказват — иначе съдържанието остава на един адрес разстояние и
   *   „скрито" е само дума.
   */
  LOOKBOOK_ENABLED: z
    .enum(['true', 'false'])
